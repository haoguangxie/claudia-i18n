...
export const MCPManager: React.FC<MCPManagerProps> = ({
  ...
  const loadServers = async () => {
    try {
      ...
      setError(t('mcp_manager.failed_to_load_servers'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleServerAdded = () => {
    loadServers();
    setToast({ message: t('mcp_manager.server_added_success'), type: "success" });
  ...
  const handleServerRemoved = (name: string) => {
    ...
    setToast({ message: t('mcp_manager.server_removed_success', { name }), type: "success" });
  };
  ...
  const handleImportCompleted = (imported: number, failed: number) => {
    ...
    if (failed === 0) {
      setToast({
        message: t('mcp_manager.import_success', { count: imported }),
        type: "success"
      });
    } else {
      setToast({
        message: t('mcp_manager.import_partial', { imported, failed }),
        type: "error"
      });
    }
  };
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
              <TabsList className="grid w-full max-w-md grid-cols-3">
                <TabsTrigger value="servers" className="gap-2">
                  ...
                  {t('mcp_manager.tab_servers')}
                </TabsTrigger>
                <TabsTrigger value="add" className="gap-2">
                  <Plus className="h-4 w-4 text-green-500" />
                  {t('mcp_manager.tab_add')}
                </TabsTrigger>
                <TabsTrigger value="import" className="gap-2">
                  <Download className="h-4 w-4 text-purple-500" />
                  {t('mcp_manager.tab_import_export')}
                </TabsTrigger>
              </TabsList>
