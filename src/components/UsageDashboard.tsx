      ...
      >
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-lg font-semibold">{t('usage_dashboard.title')}</h1>
              <p className="text-xs text-muted-foreground">
                {t('usage_dashboard.subtitle')}
              </p>
            </div>
          </div>
            ...
            <div className="flex space-x-1">
              {(["all", "30d", "7d"] as const).map((range) => (
                <Button
                  ...
                  className="text-xs"
                >
                  {range === "all" ? t('usage_dashboard.all_time') : range === "7d" ? t('usage_dashboard.last_7_days') : t('usage_dashboard.last_30_days')}
                </Button>
              ))}
            </div>
          </div>
        ...
        {loading ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center">
              ...
              <p className="text-sm text-muted-foreground">{t('usage_dashboard.loading_stats')}</p>
            </div>
          </div>
        ) : error ? (
          <div className="flex items-center justify-center h-full">
            <div className="text-center max-w-md">
              <p className="text-sm text-destructive mb-4">{t('usage_dashboard.load_error')}</p>
              <Button onClick={loadUsageStats} size="sm">
                {t('usage_dashboard.try_again')}
              </Button>
            </div>
          </div>
        ) : stats ? (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage_dashboard.total_cost')}</p>
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage_dashboard.total_sessions')}</p>
                    <p className="text-2xl font-bold mt-1">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage_dashboard.total_tokens')}</p>
                    <p className="text-2xl font-bold mt-1">
              ...
              <Card className="p-4 shimmer-hover">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-muted-foreground">{t('usage_dashboard.avg_cost_per_session')}</p>
                    <p className="text-2xl font-bold mt-1">
                      {formatCurrency(
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              <TabsList className="grid w-full grid-cols-5">
                <TabsTrigger value="overview">{t('usage_dashboard.tab_overview')}</TabsTrigger>
                <TabsTrigger value="models">{t('usage_dashboard.tab_models')}</TabsTrigger>
                <TabsTrigger value="projects">{t('usage_dashboard.tab_projects')}</TabsTrigger>
                <TabsTrigger value="sessions">{t('usage_dashboard.tab_sessions')}</TabsTrigger>
                <TabsTrigger value="timeline">{t('usage_dashboard.tab_timeline')}</TabsTrigger>
              </TabsList>

              {/* Overview Tab */}
              <TabsContent value="overview" className="space-y-4">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.token_breakdown')}</h3>
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage_dashboard.input_tokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_input_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage_dashboard.output_tokens')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_output_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage_dashboard.cache_write')}</p>
                      <p className="text-lg font-semibold">{formatTokens(stats.total_cache_creation_tokens)}</p>
                    </div>
                    <div>
                      <p className="text-xs text-muted-foreground">{t('usage_dashboard.cache_read')}</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.most_used_models')}</h3>
                        ...
                        <div key={model.model} className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {model.session_count} {t('usage_dashboard.sessions')}
                            </span>
                          </div>
                  ...
                  <Card className="p-6">
                    <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.top_projects')}</h3>
                    <div className="space-y-3">
                      {stats.by_project.slice(0, 3).map((project) => (
                        <div key={project.project_path} className="flex items-center justify-between">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {project.session_count} {t('usage_dashboard.sessions')}
                            </span>
                          </div>
                          <span className="text-sm font-medium">
              ...
              <TabsContent value="models">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.usage_by_model')}</h3>
                    ...
                    {stats.by_model.map((model) => (
                      <div key={model.model} className="space-y-2">
                        ...
                        <div className="grid grid-cols-4 gap-2 text-xs">
                          <div>
                            <span className="text-muted-foreground">{t('usage_dashboard.input')}: </span>
                            <span className="font-medium">{formatTokens(model.input_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage_dashboard.output')}: </span>
                            <span className="font-medium">{formatTokens(model.output_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage_dashboard.cache_w')}: </span>
                            <span className="font-medium">{formatTokens(model.cache_creation_tokens)}</span>
                          </div>
                          <div>
                            <span className="text-muted-foreground">{t('usage_dashboard.cache_r')}: </span>
              ...
              <TabsContent value="projects">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.usage_by_project')}</h3>
                  <div className="space-y-3">
                      ...
                      <div key={project.project_path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                        <div className="flex flex-col truncate">
                          ...
                          <div className="flex items-center space-x-3 mt-1">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {formatTokens(project.total_tokens)} {t('usage_dashboard.tokens')}
                            </span>
                          </div>
                        </div>
                        <div className="text-right">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {formatCurrency(project.total_cost / project.session_count)}/{t('usage_dashboard.session')}
                          </p>
                        </div>
                      </div>
                    ))}
              ...
              <TabsContent value="sessions">
                  <Card className="p-6">
                      <h3 className="text-sm font-semibold mb-4">{t('usage_dashboard.usage_by_session')}</h3>
                      <div className="space-y-3">
              ...
              <TabsContent value="timeline">
                <Card className="p-6">
                  <h3 className="text-sm font-semibold mb-6 flex items-center space-x-2">
                    ...
                    <span>{t('usage_dashboard.daily_usage')}</span>
                  </h3>
                    ...
                    return (
                      <div className="relative pl-8 pr-4">
                                ...
                                <div className="absolute bottom-full mb-2 left-1/2 transform -translate-x-1/2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                                  <div className="bg-background border border-border rounded-lg shadow-lg p-3 whitespace-nowrap">
                                    ...
                                    <p className="text-sm text-muted-foreground mt-1">
                                      {t('usage_dashboard.cost')}: {formatCurrency(day.total_cost)}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {formatTokens(day.total_tokens)} {t('usage_dashboard.tokens')}
                                    </p>
                                    <p className="text-xs text-muted-foreground">
                                      {day.models_used.length} {t('usage_dashboard.models', { count: day.models_used.length })}
                                    </p>
                                  </div>
                        ...
                        <div className="mt-8 text-center text-xs text-muted-foreground">
                          {t('usage_dashboard.daily_usage_over_time')}
                        </div>
                      </div>
                    )
                  })() : (
                    <div className="text-center py-8 text-sm text-muted-foreground">
                      {t('usage_dashboard.no_usage_data')}
                    </div>
                  )}
                </Card>
              </TabsContent>
