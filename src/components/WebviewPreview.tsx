...
const WebviewPreviewComponent: React.FC<WebviewPreviewProps> = ({
  ...
  return (
    <div 
      ...
      tabIndex={-1}
      role="region"
      aria-label={t('webview_preview.aria_label')}
    >
      {/* Browser Top Bar */}
      <div className="border-b bg-muted/30 flex-shrink-0">
        ...
        <div className="flex items-center justify-between px-3 py-2 border-b">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium">{t('webview_preview.title')}</span>
            {isLoading && (
            ...
            {onToggleMaximize && (
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    {isMaximized ? t('webview_preview.exit_fullscreen') : t('webview_preview.enter_fullscreen')}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            )}
        ...
        <div className="flex items-center gap-2 px-3 py-2">
          ...
          <div className="flex-1 relative">
            <Input
              ...
              placeholder={t('webview_preview.input_placeholder')}
      ...
      <div className="flex-1 relative bg-background" ref={contentRef}>
          ...
          {isLoading && (
            ...
            >
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">{t('webview_preview.loading')}</p>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        {hasError ? (
          <div className="flex flex-col items-center justify-center h-full p-8">
            ...
            <h3 className="text-lg font-semibold mb-2">{t('webview_preview.failed_to_load')}</h3>
            <p className="text-sm text-muted-foreground text-center mb-4">
              {errorMessage || t('webview_preview.load_error_tip')}
            </p>
            <Button onClick={handleRefresh} variant="outline" size="sm">
              {t('webview_preview.try_again')}
            </Button>
          </div>
        ) : currentUrl ? (
          ...
          <div className="flex flex-col items-center justify-center h-full p-8 text-foreground">
            ...
            <h3 className="text-xl font-semibold mb-3">{t('webview_preview.empty_title')}</h3>
            <p className="text-sm text-muted-foreground text-center mb-6 max-w-md">
              {t('webview_preview.empty_tip')}
            </p>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>{t('webview_preview.try_entering')}</span>
              <code className="px-2 py-1 bg-muted/50 text-foreground rounded font-mono text-xs">localhost:3000</code>
              <span>{t('webview_preview.or_any_url')}</span>
            </div>
          </div>
        )}
      </div>
