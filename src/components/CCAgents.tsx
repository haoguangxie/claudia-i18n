...
export const CCAgents: React.FC<CCAgentsProps> = ({ onBack, className }) => {
  ...
  const loadAgents = async () => {
    try {
      ...
      setToast({ message: t('ccagents.agent_load_failed'), type: "error" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const confirmDeleteAgent = async () => {
    ...
    try {
      ...
      setToast({ message: t('ccagents.agent_deleted_success'), type: "success" });
      await loadAgents();
      await loadRuns(); // Reload runs as they might be affected
    } catch (err) {
      console.error("Failed to delete agent:", err);
      setToast({ message: t('ccagents.agent_deleted_failed'), type: "error" });
    } finally {
      setIsDeleting(false);
  ...
  const handleAgentCreated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('ccagents.agent_created_success'), type: "success" });
  };

  const handleAgentUpdated = async () => {
    setView("list");
    await loadAgents();
    setToast({ message: t('ccagents.agent_updated_success'), type: "success" });
  };
  ...
  const handleExportAgent = async (agent: Agent) => {
    try {
      ...
      setToast({ message: t('ccagents.agent_exported_success', { name: agent.name }), type: "success" });
    } catch (err) {
      console.error("Failed to export agent:", err);
      setToast({ message: t('ccagents.agent_exported_failed'), type: "error" });
    }
  };
  ...
  const handleImportAgent = async () => {
    try {
      ...
      setToast({ message: t('ccagents.agent_imported_success'), type: "success" });
      await loadAgents();
    } catch (err) {
      console.error("Failed to import agent:", err);
      const errorMessage = err instanceof Error ? err.message : "Failed to import agent";
      setToast({ message: t('ccagents.agent_imported_failed', { error: errorMessage }), type: "error" });
    }
  };
    ...
    <div className={cn("flex flex-col h-full bg-background", className)}>
        ...
        >
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">{t('ccagents.title')}</h1>
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={handleImportAgent}>
                    ...
                    {t('ccagents.import_from_file')}
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => setShowGitHubBrowser(true)}>
                    <Globe className="h-4 w-4 mr-2" />
                    {t('ccagents.import_from_github')}
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
              <Button
              ...
              <div>
                {loading ? (
                  ...
                  <div className="flex flex-col items-center justify-center h-64 text-center">
                    ...
                    <h3 className="text-lg font-medium mb-2">{t('ccagents.no_agents')}</h3>
                    <p className="text-sm text-muted-foreground mb-4">
                      {t('ccagents.no_agents_tip')}
                    </p>
                            ...
                            <Card className="h-full hover:shadow-lg transition-shadow">
                              <CardContent className="p-6 flex flex-col items-center text-center">
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {t('ccagents.created')}: {new Date(agent.created_at).toLocaleDateString()}
                                </p>
                              </CardContent>
                              <CardFooter className="p-4 pt-0 flex justify-center gap-1 flex-wrap">
                                <Button
                                  ...
                                  title={t('ccagents.execute_agent')}
                                >
                    ...
                    {totalPages > 1 && (
                      <div className="mt-6 flex justify-center gap-2">
                        ...
                        <span className="flex items-center px-3 text-sm">
                          {t('ccagents.page')} {currentPage} {t('ccagents.of')} {totalPages}
                        </span>
                        <Button
                          size="sm"
                          variant="outline"
              ...
              {!loading && agents.length > 0 && (
                <div className="overflow-hidden">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-lg font-semibold">{t('ccagents.recent_executions')}</h2>
                  </div>
                  {runsLoading ? (
      ...
      <GitHubAgentBrowser
        ...
        onImportSuccess={async () => {
          ...
          await loadAgents();
          setToast({ message: t('ccagents.agent_imported_github_success'), type: "success" });
        }}
      />
        ...
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('ccagents.delete_agent')}
            </DialogTitle>
            <DialogDescription>
              {t('ccagents.delete_confirm', { name: agentToDelete?.name })}
            </DialogDescription>
          </DialogHeader>
